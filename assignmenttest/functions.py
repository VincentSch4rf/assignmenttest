...
class FunctionTest(BaseTest):
    ...
    def __init__(self, namespace, function_name, 
                 reference_function=None, comparator=compare_numbers, r_tol=0, a_tol=0):
    ...
    def has_return_statement(self):
        return any(
            ...
            for node in ast.walk(ast.parse(inspect.getsource(self.namespace[self.fun_name])))
        )
        ...
        try:
            with HiddenPrints():
                for _ in range(max_reruns):
                    result = self.call_function(self.namespace[self.fun_name], test_case)
